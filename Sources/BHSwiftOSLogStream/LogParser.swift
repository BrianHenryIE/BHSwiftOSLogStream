...
class LogParser {
    ...
    func parse(logLine: String) -> LogEntry? {
        ...
        guard let threadUint = UInt64(fromHex: threadString) else {
            return nil
        }

        guard let activityUInt = UInt64(fromHex: activityString) else {
            return nil
        }

        guard let pidInt = UInt(pidString) else {
            return nil
        }

        guard let ttlInt = UInt(ttlString) else {
            return nil
        }
